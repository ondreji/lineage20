...
public class PackageParser {
    ...
    private boolean parseBaseApplication(Package owner, Resources res,
        ...
        //add 这里我判断如果非系统应用才增加这些权限
        if((ai.flags & ApplicationInfo.FLAG_SYSTEM)!=1){
            //试错中发现，开启这些权限的时候，有个google和message相关的进程会崩溃，所以过滤一下
            if(ai.packageName.startsWith("com.android.")||
                ai.packageName.startsWith("org.android.")||
                ai.packageName.startsWith("cn.android.")||
                ai.packageName.contains("google") ||
                ai.packageName.contains("message")||
                ai.packageName.toLowerCase().contains("magisk")){
                Slog.w("MikRom", ai.packageName+" : 跳过 开启 debuggable");
            }else{ //下面是sdcard权限开启，以及debuggable权限开启。如果担心检测，可以关掉debuggable的默认开启
